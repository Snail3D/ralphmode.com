...
Update the README to reflect current progress. Keep it fresh and useful."""
    ...
    def office_chatter(self, situation: str, task_title: str = ""):
        ...
        # Also send to dashboard (parse the message for characters)
        # Format: *Character action* "dialogue"
        import re
        parts = re.findall(r'\*([^*]+)\*\s*"([^"]+)"', msg)
        for part in parts:
            action_text = part[0]
            dialogue = part[1]
            # Extract character from action
            char_match = re.match(r'(\w+)', action_text)
            if char_match:
                character = char_match.group(1)
                action = action_text.replace(character, '').strip()
                send_to_dashboard(character, dialogue, action)
