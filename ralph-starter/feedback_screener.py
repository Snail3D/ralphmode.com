...
def screen_feedback(content: str, user_id: int, feedback_id: Optional[int] = None) -> Tuple[bool, Optional[str]]:
    """
    Screen feedback for spam patterns.

    Convenience function that checks spam and auto-rejects if needed.

    Args:
        content: Feedback content
        user_id: Database user ID
        feedback_id: Optional feedback ID (for auto-rejection)

    Returns:
        Tuple of (is_spam: bool, reason: str or None)
    """
    detector = get_spam_detector()
    is_spam, reason = detector.detect_spam(content, user_id)

    # Auto-reject if spam detected and feedback_id provided
    if is_spam and feedback_id:
        detector.auto_reject_spam(feedback_id, reason)
